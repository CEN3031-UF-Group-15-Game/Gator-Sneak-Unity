...
public class DK_AssignCloakStackedOverlays : MonoBehaviour {
	...
	public static void AssignStackedOverlays ( DK_UMACrowd Crowd, List<DKSlotData> TmpSlotDataList, List<UMA.SlotData> TmpUMASlotDataList, DK_RPG_UMA _DK_RPG_UMA, DKOverlayData Overlay, string Opt, string type, int index ){
		...
		if ( type.Contains("Cloak") == true ) {
			VerifyUMAMaterial ( _DK_RPG_UMA._Equipment._Cloak.Slot._UMA, Overlay._UMA );
			// set color
			if ( Opt == "Opt01" ){
				if ( Overlay.Opt01.ColorPresets.Count > 0 && _DK_RPG_UMA._Equipment._Cloak.Opt01Color == null ) 
					_DK_RPG_UMA._Equipment._Cloak.Opt01Color 
					= Overlay.Opt01.ColorPresets [UnityEngine.Random.Range(0,Overlay.Opt01.ColorPresets.Count-1)];

				stacked = Overlay.Opt01;
				ColorToApply = _DK_RPG_UMA._Equipment._Cloak.Opt01Color.PresetColor;
			}
			if ( Opt == "Opt02" ){
				if ( Overlay.Opt02.ColorPresets.Count > 0 && _DK_RPG_UMA._Equipment._Cloak.Opt02Color == null ) 
					_DK_RPG_UMA._Equipment._Cloak.Opt02Color 
					= Overlay.Opt02.ColorPresets [UnityEngine.Random.Range(0,Overlay.Opt02.ColorPresets.Count-1)];

				stacked = Overlay.Opt02;
				ColorToApply = _DK_RPG_UMA._Equipment._Cloak.Opt02Color.PresetColor;
			}
			// Dirt
			if ( Opt == "Dirt01" ){
				if ( Overlay.Dirt01.ColorPresets.Count > 0 && _DK_RPG_UMA._Equipment._Cloak.Dirt01Color == null ) 
					_DK_RPG_UMA._Equipment._Cloak.Dirt01Color 
					= Overlay.Dirt01.ColorPresets [UnityEngine.Random.Range(0,Overlay.Dirt01.ColorPresets.Count-1)];

				stacked = Overlay.Dirt01;
				ColorToApply = _DK_RPG_UMA._Equipment._Cloak.Dirt01Color.PresetColor;
			}
			if ( Opt == "Dirt02" ){
				if ( Overlay.Dirt02.ColorPresets.Count > 0 && _DK_RPG_UMA._Equipment._Cloak.Dirt02Color == null ) 
					_DK_RPG_UMA._Equipment._Cloak.Dirt02Color 
					= Overlay.Dirt02.ColorPresets [UnityEngine.Random.Range(0,Overlay.Dirt02.ColorPresets.Count-1)];

				stacked = Overlay.Dirt02;
				ColorToApply = _DK_RPG_UMA._Equipment._Cloak.Dirt02Color.PresetColor;
			}
		}
