...
public class DK_UMA_RPG_EquipCollar : MonoBehaviour {
	...
	public static void EquipSlotElement ( DKSlotData _slot, DKOverlayData _overlay, DK_RPG_UMA _DK_RPG_UMA, ColorPresetData ColorPreset, Color color, string Layer, string LayersAction, ColorPresetData Opt1,  ColorPresetData Opt2 ){
		...
		if ( _slot != null ){
			...
			// for Sub Layer
			if ( Layer == "Sub" ){
				_DK_RPG_UMA._Equipment._CollarSub.Slot = _slot;
				// if the overlay is already assigned
				if ( _overlay ) {
					_DK_RPG_UMA._Equipment._CollarSub.Overlay = _overlay;
					...
					if ( _overlay 
						...
						if (  ColorPreset == null ){
							...
							_DK_RPG_UMA._Equipment._CollarSub.ColorPreset = _overlay.ColorPresets[ran2];
						}
						else {
							color = ColorPreset.PresetColor;
							_DK_RPG_UMA._Equipment._CollarSub.ColorPreset = ColorPreset;
						}
					}
				}
				// if the overlay is not assigned, random one from the linked overlays of the slot
					...
					if ( _slot.LinkedOverlayList.Count > 0 ) {
						...
						_DK_RPG_UMA._Equipment._CollarSub.Overlay = _slot.LinkedOverlayList[ran];
				...
				if ( color != Color.black ) _DK_RPG_UMA._Equipment._CollarSub.Color = color;
				// opt 1 & 2 colors
				_DK_RPG_UMA._Equipment._CollarSub.Opt01Color = Opt1;
				_DK_RPG_UMA._Equipment._CollarSub.Opt02Color = Opt2;
			}
			// for Main Layer
			if ( Layer == "Main" ){
				...
				if ( _overlay ) {
					...
					if ( _overlay 
						...
						if (  ColorPreset == null ){
							int ran2 = Random.Range(0, _overlay.ColorPresets.Count-1);
							color = _overlay.ColorPresets[ran2].PresetColor;
							_DK_RPG_UMA._Equipment._Collar.ColorPreset = _overlay.ColorPresets[ran2];
						}
						else {
							color = ColorPreset.PresetColor;
							_DK_RPG_UMA._Equipment._Collar.ColorPreset = ColorPreset;
						}
					}
				}
				// if the overlay is not assigned, random one from the linked overlays of the slot
				...
				// opt 1 & 2 colors
				_DK_RPG_UMA._Equipment._Collar.Opt01Color = Opt1;
				_DK_RPG_UMA._Equipment._Collar.Opt02Color = Opt2;
			}
			// for cover layer
			else if ( Layer == "Cover" ){
				_DK_RPG_UMA._Equipment._CollarCover.Slot = _slot;
				// if the overlay is already assigned
				if ( _overlay ) {
					_DK_RPG_UMA._Equipment._CollarCover.Overlay = _overlay;
					...
					if ( _overlay 
						...
						if (  ColorPreset == null ){
							int ran2 = Random.Range(0, _overlay.ColorPresets.Count-1);
							color = _overlay.ColorPresets[ran2].PresetColor;
							_DK_RPG_UMA._Equipment._CollarCover.ColorPreset = _overlay.ColorPresets[ran2];
						}
						else {
							color = ColorPreset.PresetColor;
							_DK_RPG_UMA._Equipment._CollarCover.ColorPreset = ColorPreset;
						}
					}
				}
				// if the overlay is not assigned, random one from the linked overlays of the slot
					...
					if ( _slot.LinkedOverlayList.Count > 0 ) {
						int ran = Random.Range(0, _slot.LinkedOverlayList.Count-1);
						_DK_RPG_UMA._Equipment._CollarCover.Overlay = _slot.LinkedOverlayList[ran];
						if ( _slot.LinkedOverlayList[ran] 
						    && _slot.LinkedOverlayList[ran].ColorPresets.Count > 0 ) {
							if (  ColorPreset == null ){
								int ran2 = Random.Range(0, _slot.LinkedOverlayList[ran].ColorPresets.Count-1);
				...
				if ( color != Color.black ) _DK_RPG_UMA._Equipment._CollarCover.Color = color;
				// opt 1 & 2 colors
				_DK_RPG_UMA._Equipment._CollarCover.Opt01Color = Opt1;
				_DK_RPG_UMA._Equipment._CollarCover.Opt02Color = Opt2;
			}
			#endregion Equipment
		}
