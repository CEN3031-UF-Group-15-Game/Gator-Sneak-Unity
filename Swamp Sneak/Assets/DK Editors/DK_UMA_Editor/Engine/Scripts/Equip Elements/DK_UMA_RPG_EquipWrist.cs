...
public class DK_UMA_RPG_EquipWrist : MonoBehaviour {
	...
	public static void EquipSlotElement ( DKSlotData _slot, DKOverlayData _overlay, DK_RPG_UMA _DK_RPG_UMA, ColorPresetData ColorPreset, Color color, string Layer, string LayersAction, ColorPresetData Opt1,  ColorPresetData Opt2 ){
		...
		if ( _slot != null ){
			...
			if ( Layer == "Main" ){
				...
				if ( _overlay ) {
					...
					if ( _overlay 
						...
						if (  ColorPreset == null ){
							...
							_DK_RPG_UMA._Equipment._Wrist.ColorPreset = _overlay.ColorPresets[ran2];
						}
						else {
							color = ColorPreset.PresetColor;
							_DK_RPG_UMA._Equipment._Wrist.ColorPreset = ColorPreset;
						}
					}
				}
				...
				// opt 1 & 2 colors
				_DK_RPG_UMA._Equipment._Wrist.Opt01Color = Opt1;
				_DK_RPG_UMA._Equipment._Wrist.Opt02Color = Opt2;
			}
			// for cover layer
			else if ( Layer == "Cover" ){
				_DK_RPG_UMA._Equipment._WristCover.Slot = _slot;
				// if the overlay is already assigned
				if ( _overlay ) {
					_DK_RPG_UMA._Equipment._WristCover.Overlay = _overlay;
					...
					if ( _overlay 
						...
						if (  ColorPreset == null ){
							int ran2 = Random.Range(0, _overlay.ColorPresets.Count-1);
							color = _overlay.ColorPresets[ran2].PresetColor;
							_DK_RPG_UMA._Equipment._WristCover.ColorPreset = _overlay.ColorPresets[ran2];
						}
						else {
							color = ColorPreset.PresetColor;
							_DK_RPG_UMA._Equipment._WristCover.ColorPreset = ColorPreset;
						}
					}
				}
				// if the overlay is not assigned, random one from the linked overlays of the slot
					...
					if ( _slot.LinkedOverlayList.Count > 0 ) {
						int ran = Random.Range(0, _slot.LinkedOverlayList.Count-1);
						_DK_RPG_UMA._Equipment._WristCover.Overlay = _slot.LinkedOverlayList[ran];
						if ( _slot.LinkedOverlayList[ran] 
						    && _slot.LinkedOverlayList[ran].ColorPresets.Count > 0 ) {
							if (  ColorPreset == null ){
								int ran2 = Random.Range(0, _slot.LinkedOverlayList[ran].ColorPresets.Count-1);
				...
				if ( color != Color.black ) _DK_RPG_UMA._Equipment._WristCover.Color = color;
				// opt 1 & 2 colors
				_DK_RPG_UMA._Equipment._WristCover.Opt01Color = Opt1;
				_DK_RPG_UMA._Equipment._WristCover.Opt02Color = Opt2;
			}
			#endregion Equipment
		}
		// for an Overlay element
			...
			if ( Layer == "SubOverlayOnly" ) {
				...
				if ( _overlay 
					...
					if (  ColorPreset == null ){
						int ran2 = Random.Range(0, _overlay.ColorPresets.Count-1);
						color = _overlay.ColorPresets[ran2].PresetColor;
						_DK_RPG_UMA._Equipment._WristSub.ColorPreset = _overlay.ColorPresets[ran2];
					}
					else {
						color = ColorPreset.PresetColor;
						_DK_RPG_UMA._Equipment._WristSub.ColorPreset = ColorPreset;
					}
				}
				_DK_RPG_UMA._Equipment._WristSub.Slot = null;
				_DK_RPG_UMA._Equipment._WristSub.Overlay = _overlay;
				if ( color != Color.black ) _DK_RPG_UMA._Equipment._WristSub.Color = color;
				// opt 1 & 2 colors
				_DK_RPG_UMA._Equipment._WristSub.Opt01Color = Opt1;
				_DK_RPG_UMA._Equipment._WristSub.Opt02Color = Opt2;
			}
			if ( Layer == "Main" ) {
				// color preset
				if ( _overlay 
			...
			if ( Layer == "Cover" ) {
				...
				_DK_RPG_UMA._Equipment._WristCover.Slot = null;
				_DK_RPG_UMA._Equipment._WristCover.Overlay = _overlay;
				if ( color != Color.black ) _DK_RPG_UMA._Equipment._WristCover.Color = color;
				// opt 1 & 2 colors
				_DK_RPG_UMA._Equipment._WristCover.Opt01Color = Opt1;
	...
	public static void RemoveSub ( DK_RPG_UMA _DK_RPG_UMA ){
		_DK_RPG_UMA._Equipment._WristSub.Slot = null;
		_DK_RPG_UMA._Equipment._WristSub.Overlay = null;
		_DK_RPG_UMA._Equipment._WristSub.ColorPreset = null;
	}

	public static void RemoveMain ( DK_RPG_UMA _DK_RPG_UMA ){
		_DK_RPG_UMA._Equipment._Wrist.Slot = null;
		_DK_RPG_UMA._Equipment._Wrist.Overlay = null;
		_DK_RPG_UMA._Equipment._Wrist.ColorPreset = null;
	}

	public static void RemoveCover ( DK_RPG_UMA _DK_RPG_UMA ){
		_DK_RPG_UMA._Equipment._WristCover.Slot = null;
		_DK_RPG_UMA._Equipment._WristCover.Overlay = null;
		_DK_RPG_UMA._Equipment._WristCover.ColorPreset = null;
	}
}
