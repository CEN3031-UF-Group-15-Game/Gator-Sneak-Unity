...
public class DK_UMA_RPG_EquipMask : MonoBehaviour {
	...
	public static void EquipSlotElement ( DKSlotData _slot, DKOverlayData _overlay, DK_RPG_UMA _DK_RPG_UMA, ColorPresetData ColorPreset, Color color, string Layer, string LayersAction, ColorPresetData Opt1,  ColorPresetData Opt2 ){
		...
		if ( _slot != null ){
			...
			_DK_RPG_UMA._Equipment._Mask.Slot = _slot;
			// if the overlay is already assigned
			if ( _overlay ) {
				_DK_RPG_UMA._Equipment._Mask.Overlay = _overlay;
				...
				if ( _overlay 
					...
					if (  ColorPreset == null ){
						...
						_DK_RPG_UMA._Equipment._Mask.ColorPreset = _overlay.ColorPresets[ran2];
					}
					else {
						color = ColorPreset.PresetColor;
						_DK_RPG_UMA._Equipment._Mask.ColorPreset = ColorPreset;
					}
				}
			}
			// if the overlay is not assigned, random one from the linked overlays of the slot
				...
				if ( _slot.LinkedOverlayList.Count > 0 ) {
					...
					_DK_RPG_UMA._Equipment._Mask.Overlay = _slot.LinkedOverlayList[ran];
			...
			if ( color != Color.black ) _DK_RPG_UMA._Equipment._Mask.Color = color;
			// opt 1 & 2 colors
			_DK_RPG_UMA._Equipment._Mask.Opt01Color = Opt1;
			_DK_RPG_UMA._Equipment._Mask.Opt02Color = Opt2;
			#endregion Equipment
